-- Add a platform discriminator to `runs`, generalize the host snapshot so a
-- run can come from an Android phone or a Linux workstation, and let
-- `executable_sha256` be null when the binary's identity was not preserved. See
-- `specs/benchmark-schema` - "Host state is captured as a platform-specific
-- immutable snapshot".
--
-- The Android-only columns (BSP, SUMD driver, battery, temperatures,
-- throttling, pinned clocks) become nullable; a CHECK constraint requires
-- them on `android` rows and forbids them on `linux` rows, and the mirror
-- image for the new `host_*` Linux columns, so the typed Rust model and the
-- database agree about which fields every platform carries. SQLite cannot
-- alter nullability in place, so the table is rebuilt; every existing row
-- is an Android run and is carried over as such.
...
SELECT
    id, started_at, finished_at, repetition, command_args, command_line,
    input_parameters, env_vars, env_allowlist_version, collector_version,
    'android', device_serial, device_uptime_seconds, thermal_throttling,
    bsp_version, sumd_driver_version, battery_charging,
    initial_temperature_celsius, max_temperature_celsius,
    gpu_clock_mhz, mif_clock_mhz, int_clock_mhz,
    git_commit_sha, git_dirty, git_branch, git_commit_timestamp, git_commit_subject,
